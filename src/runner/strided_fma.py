...
def run_strided_fma(cases, strides, gpu_for_filename, show_plot=False):
    benchmark = 'strided-fma'
    print(f'Running {benchmark} benchmark ...')

    # sizes_to_bench = [1024, 4096] # dummy test input for debugging
    sizes_to_bench = []
    for i in range(1_0, 20_0 + 1, 1):
    ...
    for base_case in cases:
        for stride in strides:
            ...
            for (i, size) in enumerate(sizes_to_bench):
                ...
                compute.append(re.findall(r'compute: *(\d+(?:\.\d+)?|\d+(?:\.\d+)?e-\d+) GFLOP/s', out)[0])
